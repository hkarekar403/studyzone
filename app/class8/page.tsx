...
function renderMath(text: string): React.ReactNode {
  // Split on $...$, render math segments with KaTeX
  ...
  return parts.map((part, i) => {
    if (part.startsWith('$') && part.endsWith('$')) {
      ...
      try {
        ...
        return <span key={i}>{mathStr}</span>;
      }
    }
...
export default function MathQuiz() {
  ...
  useEffect(() => {
    // @ts-ignore: CSS dynamic import is valid in browsers
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(20)
    doc.text("Class 8 Mathematics Practice Session Report", 20, 20)
  ...
  const buildWorksheetPDF = (
    ...
    doc.text('Class 8 Mathematics Worksheet', 105, 20, { align: 'center' })
  ...
  const buildAnswerKeyPDF = (
    ...
    doc.setFontSize(16)
    doc.setFont('helvetica', 'bold')
    doc.text('Answer Key — Class 8 Mathematics Worksheet', 105, 20, { align: 'center' })
    ...
    <div className="min-h-screen">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8">
              <div className="flex-1">
                ...
                <p className="text-white/80 text-lg mb-6 leading-relaxed">
                  Interactive practice for Class 8 · CBSE · ICSE · IGCSE
                </p>
        ...
        <div ref={faqRef} className="max-w-6xl mx-auto mb-8 bg-white/60 rounded-2xl p-6">
          ...
          <div>
            {[
              ...
              { q: "What age group is this for?", a: "This app is designed for Class 8 students, typically aged 13-14 years. The Easy difficulty is suitable for beginners while Hard questions challenge advanced learners." },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="rounded-xl shadow-sm border-l-4 border-purple-500 bg-purple-50 p-3">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                For fraction answers, enter in lowest terms (e.g. 3/4 not 6/8)
              </p>
            </div>
        ...
        <footer className="pb-6 px-1 flex flex-col gap-2">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <p className="text-sm text-gray-500">
              Built with ❤️ for curious minds · Class 8 Mathematics
            </p>
            {visitorCount > 0 && (
              <p className="text-sm text-gray-500">
      ...
      {showSummary && (() => {
        ...
        const handleShare = async () => {
          const shareText = `I scored ${correctAnswers}/${questionsGenerated} (${accuracy}%) in Class 8 ${curriculum} Maths on StudyZone! 🎯`
          const nav = navigator as Navigator & { share?: (d: ShareData) => Promise<void> }
          if (nav.share) {
            try {
              await nav.share({ title: "My Maths Score on StudyZone", text: shareText, url: "https://studyzone.co.in" })
